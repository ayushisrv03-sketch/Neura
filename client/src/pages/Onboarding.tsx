    ...
    <main className={`min-h-screen bg-[#faf8ff] text-[#131b2e] antialiased transition-colors ${dyslexiaFont ? 'dyslexia-font' : ''}`}>
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-col items-center px-4 py-8 sm:px-6">
        ...
        <section className="w-full rounded-2xl border border-[#e2e7ff]/60 bg-white p-6 shadow-[0_4px_24px_-2px_rgba(15,23,42,.05)] sm:p-10">
          ...
          {step === 2 && (
            <StepShell eyebrow="Step 2 • Exploration" title="What would you like to learn?" description="Choose the topics that spark your curiosity. You can choose one or both subjects anytime.">
              <div className="grid gap-3.5 sm:grid-cols-2">
                {subjects.map((item) => {
                  const selected = selectedSubjects.includes(item.name);
                  return (
                    <SelectableCard key={item.name} selected={selected} onClick={() => handleSubjectToggle(item.name)}>
                      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-100 text-lg text-sky-600">{item.icon}</span>
                      <span className="min-w-0 flex-1">
                        <strong className="block text-base">{item.name}</strong>
                        <small className="mt-0.5 block text-xs text-[#40474f]">{item.detail}</small>
                        <em className="mt-2 inline-block rounded-md bg-sky-100 px-2 py-0.5 text-[11px] not-italic text-sky-700">{item.tag}</em>
                      </span>
          ...
          {step === 5 && (
            <StepShell eyebrow="✦  Step 5 • Ready to learn" title="Your space is prepared!" description="We've shaped your personal environment around your natural strengths.">
              <div className="flex items-start gap-4 rounded-2xl border border-sky-200/80 bg-gradient-to-r from-sky-100/50 via-[#f2f3ff] to-sky-50/80 p-5 sm:items-center sm:p-6">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white text-2xl text-sky-600 shadow-sm">♥</span>
                <div>
                  <strong className="block text-xs text-sky-800">✓ Our Neuro-Affirming Promise</strong>
                  <p className="mt-1 text-base text-[#40474f]">Your learning space will adapt to you—not the other way around.</p>
                </div>
              </div>
              <div className="grid gap-3 pt-1 sm:grid-cols-2">
                <Summary title="Learner" value={name || 'Your name'} />
                <Summary title="Curriculum" value={subjectSummary} />
                <Summary title="Formats" value={`${selectedFormats.length || 0} selected`} />
              ...
              <div className="flex flex-col items-center justify-between gap-3 border-t border-[#e2e7ff]/60 pt-6 sm:flex-row">
                <button type="button" onClick={() => setStep(4)} className="rounded-full bg-[#eaedff] px-5 py-2.5 text-sm font-medium transition hover:bg-[#e2e7ff]">← Back</button>
                <button type="button" onClick={handleFinish} className="rounded-full bg-sky-600 px-7 py-3 text-base font-medium text-white shadow-sm transition hover:bg-sky-700">Enter my learning space →</button>
              </div>
            </StepShell>
          )}
        </section>
