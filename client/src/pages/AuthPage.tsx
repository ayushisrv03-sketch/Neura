...
export default function AuthPage({ defaultTab }: { defaultTab: Tab }) {
  ...
  function switchTab(next: Tab) {
    ...
    setNotice(null);
    setError(null);
    setLocation(next === "login" ? "/login" : "/signup", { replace: true });
  }
  ...
  async function handleSignupSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (password !== confirmPassword) {
      setError("Those passwords don't match yet — give it another try.");
      return;
    }
    setSubmitting(true);
    try {
    ...
    >
      ...
      <main className="w-full flex-1 flex flex-col items-center justify-center px-4 py-8 sm:py-12">
        ...
        <div className="w-full max-w-[480px] bg-white rounded-2xl border border-[#eaedff] shadow-calm p-6 sm:p-8 relative">
            ...
            <form className="space-y-4" onSubmit={handleLoginSubmit}>
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full py-3.5 px-4 rounded-xl bg-sky-600 hover:bg-sky-700 active:bg-sky-800 text-white font-medium text-sm flex items-center justify-center gap-2 shadow-sm transition transform hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-sky-600/20 disabled:opacity-60 disabled:hover:translate-y-0"
                >
                  <span>{submitting ? "Logging in…" : "Log In to Neura"}</span>
                  {!submitting && (
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            <form className="space-y-4" onSubmit={handleSignupSubmit}>
              ...
              <div>
                ...
                <div className="mt-2 p-2.5 bg-amber-50/70 border border-amber-200/60 rounded-lg flex items-start gap-2 text-xs text-amber-800">
                  ...
                  <span>Passwords need at least 8 characters. No timed lockout if you make a typo.</span>
                </div>
              </div>
              <div className="pt-2">
                <button
          ...
          {error && (
            <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-xl text-xs text-red-700 text-center font-medium" role="alert">
              {error}
            </div>
          )}
