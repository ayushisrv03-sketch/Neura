...
const initialCourses = [
  ...
  { id: 3, title: "Geometry", subject: "Maths", level: "Grade 7", progress: 0, lessonsCompleted: 0, lessonsTotal: 14, nextLesson: "Angles and triangles", accent: "lilac", icon: "geometry" },
  { id: 4, title: "Photosynthesis", subject: "Science", level: "Grade 6", progress: 0, lessonsCompleted: 0, lessonsTotal: 11, nextLesson: "The role of sunlight", accent: "green", icon: "leaf" },
  { id: 5, title: "States of matter", subject: "Science", level: "Grade 6", progress: 0, lessonsCompleted: 0, lessonsTotal: 11, nextLesson: "Particles in motion", accent: "orange", icon: "matter" },
  { id: 6, title: "Parts of a plant", subject: "Science", level: "Grade 5", progress: 0, lessonsCompleted: 0, lessonsTotal: 11, nextLesson: "Roots and their jobs", accent: "yellow", icon: "plant" },
];
const initialTasks = [
...
export default function Home() {
  ...
  const resetProgress = trpc.dashboard.resetProgress.useMutation();
  ...
  const [focusCourseId, setFocusCourseId] = useState(1);
  ...
  const totalMinutes = sessions.reduce((sum, value) => sum + value, 0) + sessionBoost;
  ...
  const averageProgress = courses.length
    ? Math.round(courses.reduce((sum, course) => sum + (progressOverrides[course.id] ?? course.progress), 0) / courses.length)
    : 0;
  ...
  const xp = stats?.xp ?? 0;
  const weeklyXp = (stats?.weeklyXp ?? 0) + sessionBoost * 4;
  const recentActivity = stats?.recentActivity ?? [];
  const hasStarted = totalMinutes > 0 || averageProgress > 0 || xp > 0 || streak > 0 || completedTasks > 0;
  const selectedCourse = courses.find(course => course.id === focusCourseId) ?? courses[0];
  ...
  useEffect(() => {
    if (courses.length && !courses.some(course => course.id === focusCourseId)) setFocusCourseId(courses[0].id);
  ...
  const startFocusSession = () => {
    ...
    setShowFocus(false);
    setSessionBoost(current => current + focusMinutes);
    setProgressOverrides(current => ({ ...current, [selectedCourse.id]: Math.min(100, (current[selectedCourse.id] ?? selectedCourse.progress) + 3) }));
    logStudy.mutate({ courseId: selectedCourse.id, minutes: focusMinutes }, {
      onSuccess: () => {
        setSessionBoost(0);
        setProgressOverrides({});
        void utils.dashboard.overview.invalidate();
        toast.success(`${focusMinutes} minute focus session logged`, { description: `${selectedCourse.title} is moving forward.` });
      },
      onError: () => toast.error("Your session was kept locally, but could not sync yet."),
    });
  };
  ...
  return (
    <DashboardLayout allowGuest>
      <div className="min-h-screen bg-[#f6fbfd] text-[#214554]">
        <header className="sticky top-0 z-30 border-b border-[#e5f0f3] bg-[#f6fbfd]/90 backdrop-blur-xl">
          <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between gap-4 px-5 sm:px-8 lg:px-10">
            ...
            <div className="ml-auto flex items-center gap-3"><button aria-label="Notifications" onClick={() => toast("You're all caught up", { description: "No new learning notifications." })} className="relative grid h-10 w-10 place-items-center rounded-xl border border-[#e4eff2] bg-white text-[#72909c] transition hover:border-[#bde3eb] hover:text-[#159ac1]"><Bell className="h-[18px] w-[18px]" /><span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-[#f59b75]" /></button><div className="hidden h-8 w-px bg-[#e4eff2] sm:block" /><div className="flex items-center gap-2.5"><div className="grid h-10 w-10 place-items-center rounded-full bg-[#dff5fb] text-xs font-bold text-[#159ac1]">{initials(displayName)}</div><div className="hidden sm:block"><p className="text-sm font-semibold leading-tight text-[#214554]">{displayName}</p><p className="mt-0.5 text-xs text-[#8aa7b1]">{user ? "Student account" : "Demo workspace"}</p></div></div></div>
          </div>
        </header>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button onClick={() => {
                ...
                resetProgress.mutate(undefined, {
                  onSuccess: () => {
                    void utils.dashboard.overview.invalidate();
                    toast.success("Progress reset", { description: "Your stats are back at zero. Learn to build them back up." });
                  },
                  onError: () => toast.error("Could not reset progress. Try again."),
                });
              }} className="flex h-12 shrink-0 items-center justify-center gap-2 rounded-xl border border-[#d4e9ee] bg-white px-4 text-sm font-bold text-[#5a7d88] transition hover:bg-[#f2f9fb] hover:text-[#159ac1]"><RotateCcw className="h-4 w-4 text-[#159ac1]" /> Start Fresh (0%)</button>
              <button onClick={() => setShowFocus(true)} className="group flex h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#159ac1] px-5 text-sm font-bold text-white shadow-[0_10px_25px_rgba(21,154,193,0.2)] transition hover:-translate-y-0.5 hover:bg-[#1088aa] active:translate-y-0"><Play className="h-4 w-4 fill-current" /> Start a focus session <ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" /></button>
            </div>
          </section>
      ...
      {showFocus && <FocusDialog courses={courses} selectedCourseId={focusCourseId} minutes={focusMinutes} setCourse={setFocusCourseId} setMinutes={setFocusMinutes} onClose={() => setShowFocus(false)} onStart={startFocusSession} />}
    </DashboardLayout>
  );
}
...
function FocusDialog({ courses, selectedCourseId, minutes, setCourse, setMinutes, onClose, onStart }: { courses: Array<{ id: number; title: string }>; selectedCourseId: number; minutes: number; setCourse: (id: number) => void; setMinutes: (minutes: number) => void; onClose: () => void; onStart: () => void }) {
  return <div className="fixed inset-0 z-[70] grid place-items-center bg-[#173c4b]/35 p-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="focus-title"><div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-[0_25px_80px_rgba(23,60,75,0.25)] sm:p-7"><div className="flex items-start justify-between"><div><div className="mb-3 grid h-11 w-11 place-items-center rounded-2xl bg-[#e5f8fc] text-[#159ac1]"><Clock3 className="h-5 w-5" /></div><h2 id="focus-title" className="text-xl font-bold tracking-tight text-[#214554]">Start a focus session</h2><p className="mt-1 text-sm text-[#8aa7b1]">Log a little intentional learning time.</p></div><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl text-[#a4b8bf] hover:bg-[#f1f8f9]" aria-label="Close focus dialog"><X className="h-4 w-4" /></button></div><label className="mt-7 block text-xs font-bold text-[#6e8c97]">Course<select value={selectedCourseId} onChange={event => setCourse(Number(event.target.value))} className="mt-2 h-11 w-full rounded-xl border border-[#dfeef1] bg-white px-3 text-sm font-semibold text-[#315866] outline-none focus:border-[#8ad5e4]">{courses.map(course => <option key={course.id} value={course.id}>{course.title}</option>)}</select></label><div className="mt-5"><p className="text-xs font-bold text-[#6e8c97]">Session length</p><div className="mt-2 grid grid-cols-3 gap-2">{[15, 25, 45].map(option => <button key={option} onClick={() => setMinutes(option)} className={`h-11 rounded-xl border text-sm font-bold transition ${minutes === option ? "border-[#159ac1] bg-[#e8f8fc] text-[#159ac1]" : "border-[#dfeef1] text-[#7f9ba5] hover:border-[#a7dce6]"}`}>{option} min</button>)}</div></div><button onClick={onStart} className="mt-7 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#159ac1] text-sm font-bold text-white transition hover:bg-[#1088aa] active:scale-[.98]"><Play className="h-4 w-4 fill-current" /> Log focus session</button></div></div>;
}
