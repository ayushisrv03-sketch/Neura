...
export default function TopicLesson() {
  ...
  }, [location, topic, curriculum.lessons.length]);
  ...
  const completeLesson = () => {
    ...
    if (!isLastLesson) {
      ...
      setLocation(`/dashboard/lessons/${encodeURIComponent(topic)}?lesson=${nextIdx + 1}`);
            ...
            <article className="rounded-2xl border border-[#dff0f4] bg-white p-6 shadow-[0_10px_35px_rgba(27,91,109,0.04)] sm:p-8">
              ...
              <div className="mt-6 rounded-2xl border border-[#e5f0f3] p-5">
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-3">
                  {activeQ.choices.map((choice) => {
                    const isSelected = answers[currentQIdx] === choice;
                    const isCorrect = choice === activeQ.answer;
                    return (
                      <button
                        key={choice}
                        onClick={() => handleSelectAnswer(choice)}
                        className={`rounded-xl border px-3 py-3 text-left text-sm font-semibold transition ${isSelected
                            ? isCorrect
